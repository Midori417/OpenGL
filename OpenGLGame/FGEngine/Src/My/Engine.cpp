...
*/
	...
	*/
	int Engine::Initialize()
	{
		...
		prog3D = std::make_shared<ProgramObject>("standard3D", "FGEngine/Res/Shader/standard3D.vert", "FGEngine/Res/Shader/standard3D.frag");
		progShadow = std::make_shared<ProgramObject>("shadow3D", "FGEngine/Res/Shader/shadow3D.vert", "FGEngine/Res/Shader/shadow3D.frag");
		progSkeletal3D = std::make_shared<ProgramObject>("skeletal3D", "FGEngine/Res/Shader/skeletal3D.vert", "FGEngine/Res/Shader/standard3D.frag");
		progShadowSkeletal3D = std::make_shared<ProgramObject>("shadowSkeletal3D", "FGEngine/Res/Shader/shadowSkeletal3D.vert", "FGEngine/Res/Shader/shadow3D.frag");
		progUnlit = std::make_shared<ProgramObject>("unlit", "FGEngine/Res/Shader/unlit.vert", "FGEngine/Res/Shader/unlit.frag");
		...
		ImFont* font01 = io.Fonts->AddFontFromFileTTF("FGEngine/Res/Font/arial.ttf", 100);
		ImFont* font02 = io.Fonts->AddFontFromFileTTF("FGEngine/Res/Font/arial.ttf", 160);

		// SkySphereの設定
		skySphere = meshBuffer->LoadObj("FGEngine/Res/Mesh/skySphere/sky_sphere.obj");
	...
	*/
	void Engine::DrawSkySphere()
	{
		...
		if (!skySphere || !sceneManager->CurrentScene()->GetSkyMaterial())
		{
			return;
		}
		...
		// スカイスフィアを描画
		const MaterialList materials(1, sceneManager->CurrentScene()->GetSkyMaterial());
