...
#include "FGEngine/Rendering/VertexArrayObject.h"
...
namespace FGEngine
{
	...
	*/
	void MeshBuffer::AddVertexData(const void* vertices, size_t vertexBytes, const uint16_t* indices, size_t indexBytes, size_t stride, GLenum mode)
	{
		...
		// 追加した図形の描画パラメータを作成
		RenderingSystem::DrawParams newParams;
		newParams.mode = mode;
	...
	*/
	MeshBuffer::MeshData MeshBuffer::CreateMeshDataFormObj(const std::string& filename)
	{
		...
		for (; i < usemtls.size() - 1; ++i) 
		{
			...
			// 描画パラメータを作成
			RenderingSystem::DrawParams params;
	...
	*/
	void Draw(const ShaderPtr& shader, const std::vector<RenderingSystem::DrawParams>& drawParamsList, const MaterialList& materials)
	{
		// シェーダプログラムを設定
		shader->Use();
