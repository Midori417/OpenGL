...
namespace FGEngine
{
	...
	* @param indexBytes		indciesのバイト数
	*/
	void MeshBuffer::AddVertexData(const void* vertices, size_t vertexBytes, const uint16_t* indices, size_t indexBytes, size_t stride)
	{
		AddVertexData(vertices, vertexBytes, indices, indexBytes, stride, GL_TRIANGLES);
	}
	...
	*/
	std::vector<MaterialPtr> MeshBuffer::LoadMTL(const std::string& foldername, const char* filename)
	{
		...
		while (!file.eof())
		{
			...
			{
				pMaterial->mainTexture = AssetManager::GetInstance()->GetTexture("white");
			}
			// 発光色の読み取りを試みる
	...
	void Draw(const ShaderPtr& shader, const std::vector<DrawParams>& drawParamsList, const MaterialList& materials)
	{
		...
		for (const auto& e : drawParamsList)
		{
			...
			if (e.materialNo >= 0 && e.materialNo < materials.size())
			{
				...
				// 色を設定
				shader->SetColor("color", material.color);
				// 発光色を設定
				shader->SetColor("emissionColor", material.emissionColor);
				shader->SetFloat("alphaCutoff", material.alphatCutOff);
	...
	*/
	...
	{
		Draw(shader, mesh.drawParamsList, materials);
	}
	...
	*/
	void FillMissingNormals(RenderingSystem::Vertex* vertices, size_t vertexCount, const uint16_t* indices, size_t indexCount)
	{
		...
		for (int i = 0; i < indexCount; i += 3)
		{
			...
			// 法線を正規化して単位ベクトルにする
			const Vector3 normal = Vector3::Normalize(c);
